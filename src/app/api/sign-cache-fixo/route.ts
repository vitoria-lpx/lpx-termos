...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `termo-lpx-cache-fixo-${nome.trim().replace(/\s+/g, "-").toLowerCase()}.pdf`;
    ...
    console.error("[sign-cache-fixo]", err);
    return NextResponse.json(
...
function buildPDF(params: PDFParams) {
  ...
  doc.setFontSize(11);
  doc.setTextColor(20, 20, 20);
  doc.text("TERMO DE PARCERIA COMERCIAL — MODELO CACHÊ FIXO", pageW / 2, y, { align: "center" });
  y += 10;
  ...
  y = txt(doc, "As condições específicas de cada campanha, incluindo conteúdos, formatos, datas, horários, canais e demais requisitos, serão previamente acordadas entre as PARTES e integrarão as obrigações da INFLUENCIADORA.", margin, y, maxW, lh, pageH, margin);
  ...
  y = txt(doc, "A autorização é válida em território nacional e internacional e abrange todos os meios, incluindo mídias e plataformas digitais, Instagram, TikTok, Facebook, marketplaces, websites, televisão, rádio, mídia impressa, streaming e tecnologias futuras.", margin, y, maxW, lh, pageH, margin);
  y = txt(doc, "A INTERMEDIADORA poderá editar, adaptar, cortar, reproduzir, sincronizar, legendar, traduzir, remixar, reutilizar e transformar os materiais, sem autorização adicional.", margin, y, maxW, lh, pageH, margin);
  y = txt(doc, "A autorização compreende campanhas patrocinadas, tráfego pago, impulsionamento, dark posts, whitelisting, branded content e inteligência artificial, sem autorização adicional.", margin, y, maxW, lh, pageH, margin);
  ...
  // REMUNERAÇÃO
  y = sectionHeader(doc, "3. Remuneração", y, margin, pageW, pageH);
  y = txt(doc, "Pela realização e publicação dos conteúdos acordados para cada campanha, a INFLUENCIADORA fará jus ao valor fixo previamente estabelecido entre as PARTES.", margin, y, maxW, lh, pageH, margin);
  y = txt(doc, "A remuneração será apurada mensalmente, considerando as campanhas e conteúdos efetivamente realizados pela INFLUENCIADORA no mês anterior, conforme as condições previamente acordadas para cada campanha.", margin, y, maxW, lh, pageH, margin);
  y = txt(doc, "A INFLUENCIADORA deverá emitir nota fiscal no prazo de 7 dias. A não emissão será interpretada como recusa da comissão, desobrigando a INTERMEDIADORA do respectivo pagamento até que a emissão seja regularizada.", margin, y, maxW, lh, pageH, margin);
  y = txt(doc, "Prazo de pagamento: 5 dias úteis, a partir da emissão da nota fiscal, desde que cumpridas as obrigações relativas às entregas acordadas.", margin, y, maxW, lh, pageH, margin);
  ...
  // INFLUENCIADORA column — com assinatura manuscrita
  if (assinatura) {
    try {
...
<body>
  <div class="wrap">
    ...
    <div class="body">
      ...
      <p>O Termo de Parceria Comercial (Modelo Cachê Fixo) foi registrado com sucesso. O documento completo em PDF está anexado a este e-mail.</p>
      <div class="card">
